    ...
    // Sistema de Armazenamento Persistente
        ...
        class GamificationSystem {
            ...
            static checkBadges(playerData) {
                const badges = [
                    { id: 'first_study', name: 'Primeira Sessão', icon: '🎯', condition: () => playerData.completedTasks >= 1 },
                    { id: 'dedicated', name: 'Dedicado', icon: '📚', condition: () => playerData.completedTasks >= 10 },
                    { id: 'scholar', name: 'Estudioso', icon: '🎓', condition: () => playerData.studyMinutes >= 300 },
                    { id: 'streak_3', name: 'Sequência de 3', icon: '🔥', condition: () => playerData.streak >= 3 },
                    { id: 'level_5', name: 'Nível 5', icon: '⭐', condition: () => playerData.level >= 5 }
                ];
            ...
            static getBadgeInfo(badgeId) {
                const badges = {
                    'first_study': { name: 'Primeira Sessão', icon: '🎯', description: 'Completou sua primeira sessão de estudos' },
                    'dedicated': { name: 'Dedicado', icon: '📚', description: 'Completou 10 tarefas de estudo' },
                    'scholar': { name: 'Estudioso', icon: '🎓', description: 'Estudou por mais de 5 horas' },
                    'streak_3': { name: 'Sequência de 3', icon: '🔥', description: 'Manteve uma sequência de 3 dias' },
                    'level_5': { name: 'Nível 5', icon: '⭐', description: 'Alcançou o nível 5' }
                };
                return badges[badgeId] || { name: 'Desconhecida', icon: '❓', description: '' };
            }
        }
            ...
            static generateStudyPlan(exams) {
                ...
                exams.forEach(exam => {
                    ...
                    if (daysUntilExam > 0) {
                        ...
                        for (let i = 0; i < studyDays; i++) {
                            ...
                            for (let session = 0; session < sessionsPerDay; session++) {
                                ...
                                studyPlans.push({
                                    ...
                                    type: 'study-session',
                                    examId: exam.id,
                                    description: `Sessão de estudo para ${exam.title}`,
                                    autoGenerated: true,
                        ...
                        studyPlans.push({
                            id: `review_${exam.id}`,
                            title: `Revisão Final - ${exam.title}`,
                            subject: exam.subject,
                            ...
                            type: 'final-review',
                            examId: exam.id,
                            description: 'Revisão final antes da prova',
                            autoGenerated: true,
                            priority: 'high'
                        });
                    }
            ...
            static show(message, type = 'info', duration = 3000) {
                ...
                notification.innerHTML = `
                    <div class="flex items-center space-x-3">
                        ...
                        <button onclick="this.parentElement.parentElement.remove()" class="text-lg opacity-70 hover:opacity-100">×</button>
                    </div>
                `;
            ...
            static checkUpcomingExams() {
                ...
                exams.then(examList => {
                    examList.forEach(exam => {
                        ...
                        if (daysUntil === 1) {
                            this.show(`🚨 Prova amanhã: ${exam.title}`, 'warning', 5000);
                        } else if (daysUntil === 3) {
                            this.show(`📚 Prova em 3 dias: ${exam.title}`, 'info', 4000);
                        }
                    });
                });
            }
        ...
        class Database {
            ...
            static addNote(note) {
                ...
                NotificationSystem.show('Anotação salva!', 'success');
                return Promise.resolve(id);
            }
            ...
            static deleteNote(noteId) {
                ...
                if (allNotes[userId] && allNotes[userId][noteId]) {
                    ...
                    StorageManager.save('notes', allNotes);
                    NotificationSystem.show('Anotação removida!', 'info');
                    return Promise.resolve();
                }
                return Promise.reject(new Error('Anotação não encontrada'));
            }
            ...
            static getGroups() {
                ...
                if (user.role === 'teacher') {
                    ...
                    // Para alunos, buscar grupos onde estão inseridos
                    const studentGroups = [];
